 ...
 * Global variables
 *********************************************************************************************/
uint32_t const VERSION_MARKER[] PROGMEM = {
    0x5AA55AA5, 0xFFFFFFFF, 0xA55AA55A
};

struct WIFI {
    int last_tx_pwr;
    uint32_t last_event = 0;                // Last Wi-Fi connection event
    uint32_t downtime   = 0;                // Wi-Fi down duration
...
*/
    ...
    CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32P4
...
TasmotaGlobal_t TasmotaGlobal = {0};
...
void setup(void) {
    ...
    #if !defined(FIRMWARE_SAFEBOOT) && !defined(DISABLE_PSRAMCHECK) && !defined(CORE32SOLO1)
    ...
    #endif // !FIRMWARE_SAFEBOOT && !DISABLE_PSRAMCHECK && !CORE32SOLO1
    #endif // ESP32 && CONFIG_IDF_TARGET_ESP32
    ...
    #if defined(USE_EMERGENCY_RESET)
    EmergencyReset();
    ...
    if (FoundPSRAM() && !CanUsePSRAM()) {
        AddLog(LOG_LEVEL_INFO,
               PSTR("HDW: PSRAM is disabled, requires specific compilation on this hardware (see doc)"));
    }
    #endif // HAS_PSRAM_FIX
    #else  // ESP8266
    ...
    if (Settings->param[P_BOOT_LOOP_OFFSET]) { // SetOption36
        // Disable functionality as possible cause of fast restart within BOOT_LOOP_TIME seconds
        // (Exception, WDT or restarts)
        if (RtcReboot.fast_reboot_count > Settings->param[P_BOOT_LOOP_OFFSET]) { // Restart twice
            Settings->flag3.user_esp8285_enable = 0;            // SetOption51 - Enable ESP8285 user GPIO's - Disable ESP8285
                                                                // Generic GPIOs interfering with flash SPI
...
void Scheduler(void) {
    ...
    // Not needed with esp32 mdns
    #if defined(ESP8266) && defined(USE_DISCOVERY) && \
        defined(USE_WEBSERVER) && defined(WEBSERVER_ADVERTISE)
    MdnsUpdate();
    ...
    #if !defined(SYSLOG_UPDATE_SECOND)
    SyslogAsync(false);
