...
uint8_t const SWITCH_AC_PERIOD =
    (20 + SWITCH_FAST_PROBE_INTERVAL - 1) / SWITCH_FAST_PROBE_INTERVAL; // Duration of an AC wave in probe intervals
...
void SwitchProbe(void) {
    if (Switch.probe_mutex || (TasmotaGlobal.uptime < 4)) {
        return;
...
void SwitchInit(void) {
    ...
    for (uint32_t i = 0; i < MAX_SWITCHES_SET; i++) {
        ...
        if (PinUsed(GPIO_SWT1, i)) {
            ...
            #if defined(ESP8266)
            pinMode(Pin(GPIO_SWT1, i), bitRead(Switch.no_pullup_mask, i)
                                       ? INPUT
                                       : ((16 == Pin(GPIO_SWT1, i)) ? INPUT_PULLDOWN_16 : INPUT_PULLUP));
            #endif // ESP8266

            #if defined(ESP32)
            pinMode(Pin(GPIO_SWT1, i), bitRead(Switch.pulldown_mask, i)
                                       ? INPUT_PULLDOWN
                                       : bitRead(Switch.no_pullup_mask, i) ? INPUT : INPUT_PULLUP);
            #endif // ESP32
...
void SwitchHandler(void) {
    ...
    for (uint32_t i = 0; i < MAX_SWITCHES_SET; i++) {
        ...
        if (Switch.hold_timer[i] &
            (((switchmode == PUSHHOLDMULTI) | (switchmode == PUSHHOLDMULTI_INV)) ? SM_TIMER_MASK : SM_NO_TIMER_MASK)) {
            Switch.hold_timer[i]--;
            if ((Switch.hold_timer[i] & SM_TIMER_MASK) == loops_per_second * Settings->param[P_HOLD_TIME] / 25) {
                if ((switchmode == PUSHHOLDMULTI) | (switchmode == PUSHHOLDMULTI_INV)) {
                    if (((switchmode == PUSHHOLDMULTI) & (NOT_PRESSED == Switch.last_state[i])) |
                        ((switchmode == PUSHHOLDMULTI_INV) & (PRESSED == Switch.last_state[i]))) {
            ...
            if (0 == (Switch.hold_timer[i] &
                      (((switchmode == PUSHHOLDMULTI) | (switchmode == PUSHHOLDMULTI_INV)) ? SM_TIMER_MASK
                                                                                           : SM_NO_TIMER_MASK))) {
                switch (switchmode) {
                    case TOGGLEMULTI :
        ...
        if (button != Switch.last_state[i]) { // This implies if ((PRESSED == button) then (NOT_PRESSED == Switch.last_state[i]))
            switch (switchmode) {
                ...
                case PUSHBUTTONHOLD : // SwitchMode 5
                    if (PRESSED == button) {
                        Switch.hold_timer[i] =
                            loops_per_second * Settings->param[P_HOLD_TIME] / 10; // Start timer on button press
                    }
                ...
                case PUSHHOLDMULTI : // SwitchMode 11
                    if (NOT_PRESSED == button) {
                        if ((Switch.hold_timer[i] & SM_TIMER_MASK) != 0) {
                            Switch.hold_timer[i] =
                                ((Switch.hold_timer[i] & ~SM_TIMER_MASK) == SM_FIRST_PRESS) ? SM_SECOND_PRESS : 0;
                            SendKey(KEY_SWITCH, i + 1, POWER_INV); // Execute command via MQTT
                            mqtt_action = POWER_INV;
                        }
                    }
                    else {
                        if ((Switch.hold_timer[i] & SM_TIMER_MASK) >
                            loops_per_second * Settings->param[P_HOLD_TIME] / 25) {
        ...
        if (switchflag <= POWER_TOGGLE) {
            if (!Settings->flag5.mqtt_switches) { // SetOption114 (0) - Detach Switches from relays and enable MQTT action state for all the SwitchModes
                ...
                mqtt_action = switchflag;
            }
        }

        if ((mqtt_action != POWER_NONE) &&
            Settings->flag5.mqtt_switches) { // SetOption114 (0) - Detach Switches from relays and enable MQTT action state for all the SwitchModes
            if (!Settings->flag.hass_discovery) { // SetOption19 - Control Home Assistant automatic discovery (See SetOption59)
                ...
                if (mqtt_action <= 3) {
                    if (mqtt_action != 3) {
                        SendKey(KEY_SWITCH, i + 1, mqtt_action);
                    }
