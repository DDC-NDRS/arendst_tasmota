...
uint8_t const BUTTON_PROBE_INTERVAL = 10;           // Time in milliseconds between button input probe
uint8_t const BUTTON_FAST_PROBE_INTERVAL = 2;       // Time in milliseconds between button input probe for AC detection
uint8_t const BUTTON_AC_PERIOD =
    (20 + BUTTON_FAST_PROBE_INTERVAL - 1) / BUTTON_FAST_PROBE_INTERVAL; // Duration of an AC wave in probe intervals

char const kMultiPress[] PROGMEM = "|SINGLE|DOUBLE|TRIPLE|QUAD|PENTA|CLEAR|";
...
void ButtonProbe(void) {
    if (Button.probe_mutex || (TasmotaGlobal.uptime < 4)) {
        return;
    ...
    for (uint32_t i = 0; i < MAX_KEYS_SET; i++) {
        ...
        if (PinUsed(GPIO_KEY1, i)) {
            ...
            if (bitRead(TouchButton.touch_mask, i)) {
                if (ac_detect || bitRead(TouchButton.calibration, i + 1)) {
                    continue;
                } // Touch is slow. Takes 21mS to read
        ...
        if ((0 == i) && ((SONOFF_DUAL == TasmotaGlobal.module_type) || (CH4 == TasmotaGlobal.module_type))) {
            ...
            if (PinUsed(GPIO_KEY1, i)) {
                ...
                pinMode(Pin(GPIO_KEY1, i), bitRead(Button.no_pullup_mask, i)
                                           ? INPUT
                                           : ((16 == Pin(GPIO_KEY1, i)) ? INPUT_PULLDOWN_16 : INPUT_PULLUP));
                #endif // ESP8266

                #if defined(ESP32)
                pinMode(Pin(GPIO_KEY1, i), bitRead(Button.pulldown_mask, i)
                                           ? INPUT_PULLDOWN
                                           : bitRead(Button.no_pullup_mask, i) ? INPUT : INPUT_PULLUP);
                #endif // ESP32
...
void ButtonHandler(void) {
    ...
    for (uint32_t button_index = 0; button_index < MAX_KEYS_SET; button_index++) {
        if (!bitRead(Button.used, button_index)) {
            continue;
        }
        ...
        if (!button_index && ((SONOFF_DUAL == TasmotaGlobal.module_type) || (CH4 == TasmotaGlobal.module_type))) {
            if (Button.dual_code) {
                ...
                if (0xF500 == Button.dual_code) { // Button hold
                    Button.hold_timer[button_index] =
                        (loops_per_second * Settings->param[P_HOLD_TIME] / 10) - 1; // SetOption32 (40)
                    hold_time_extent = 1;
                }
                Button.dual_code = 0;
            }
            ...
            if (NOT_PRESSED == button) {
                ...
                if (Settings->flag3.mqtt_buttons && (PRESSED == Button.last_state[button_index]) &&
                    !Button.press_counter[button_index]) { // SetOption73 (0) - Decouple button from relay and send just mqtt topic
                ...
                Button.hold_timer[button_index]++;
                if (Settings->flag
                        .button_single) { // SetOption13 (0) - Allow only single button press for immediate action
                    if (Button.hold_timer[button_index] ==
                        loops_per_second * hold_time_extent * Settings->param[P_HOLD_TIME] /
                            10) { // SetOption32 (40) - Button held for factor times longer
                    ...
                    if (Button.hold_timer[button_index] ==
                        loops_per_second * Settings->param[P_HOLD_TIME] / 10) { // SetOption32 (40) - Button hold
                        ...
                        if (Settings->flag.button_restrict) {         // SetOption1 (0) - Control button multipress
                            if (Settings->param[P_HOLD_IGNORE] > 0) { // SetOption40 (0) - Do not ignore button hold
                                if (Button.hold_timer[button_index] >
                                    loops_per_second * Settings->param[P_HOLD_IGNORE] / 10) {
                            ...
                            if ((Button.hold_timer[button_index] ==
                                 loops_per_second * hold_time_extent * Settings->param[P_HOLD_TIME] / 10)) { // SetOption32 (40) - Button held for factor times longer
                                Button.press_counter[button_index] = 0;
            ...
            if (!Settings->flag.button_single) { // SetOption13 (0) - Allow multi-press
                if (Button.window_timer[button_index]) {
                    ...
                    if (!TasmotaGlobal.restart_flag && !Button.hold_timer[button_index] &&
                        (Button.press_counter[button_index] > 0) && (Button.press_counter[button_index] < 7)) {
                        ...
                        if (Button.press_counter[button_index] < 3) { // Single or Double press
                            ...
                            #endif // ESP8266
                            {
                                ...
                                if ((1 == Button.used) &&
                                    (2 == TasmotaGlobal.devices_present)) { // Single Button with two devices only
                        ...
                        if (XdrvCall(FUNC_BUTTON_MULTI_PRESSED)) {
                            ...
                            #endif
                            if (!Settings->flag3.mqtt_buttons && single_press &&
                                SendKey(KEY_BUTTON, button_index + Button.press_counter[button_index],
                                        POWER_TOGGLE)) { // Execute Toggle command via MQTT if ButtonTopic is set
                                                         // Success
                            }
                            else {
                                    ...
                                    if (!Settings->flag3.mqtt_buttons) { // SetOption73 - Detach buttons from relays and enable MQTT action state for multipress
                                        if (Button.press_counter[button_index] == 1) { // By default first press always send a TOGGLE (2)
                                            ...
                                            if (0 == button_index) { // BUTTON1 can toggle up to 5 relays if present. If a relay is not present will send out the key value (2,11,12,13 and 14) for rules
                                                uint32_t max_device =
                                                    (TasmotaGlobal.devices_present < MAX_RELAY_BUTTON1)
                                                        ? TasmotaGlobal.devices_present
                                                        : MAX_RELAY_BUTTON1;
                                                if ((Button.press_counter[button_index] > 1) &&
                                                    (Button.press_counter[button_index] <= max_device)) {
                                ...
                                if (Settings->flag3.mqtt_buttons) { // SetOption73 (0) - Decouple button from relay and send just mqtt topic
                                    if (Button.press_counter[button_index] >= 1 &&
                                        Button.press_counter[button_index] <= 5) {
...
void MqttButtonTopic(uint32_t button_id, uint32_t action, uint32_t hold) {
    ...
    if (!Settings->flag.hass_discovery) { // SetOption19 - Control Home Assistant automatic discovery (See SetOption59)
        ...
        char mqttstate[7];
        Response_P(S_JSON_SVALUE_ACTION_SVALUE, scommand,
                   (hold) ? SettingsText(SET_STATE_TXT4)
